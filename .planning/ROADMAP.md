...
|---|-------|------|--------------|-------|
| 1 | Foundation | Authenticated user can create a workspace and invite members, with multi-tenant isolation enforced from the first migration | AUTH-01–05, WORK-01–06, TEST-01 | TBD |
...
**Plans:**
- [ ] DB Schema & Docker: PostgreSQL schema with RLS, fractional indexing (`position DOUBLE PRECISION`), `stage_entered_at`, and Docker Compose (db + api + frontend)
- [ ] Backend Auth & Tenant API: .NET 9 SSO handlers (GitHub/Google/Microsoft), EF Core global query filters, Workspace + Membership endpoints, xUnit + TestContainers wired for TDD
- [ ] Frontend Auth & Workspace UI: Next.js App Router + Auth.js v5, SSO login page, workspace creation flow, member invite + accept flow, member list management
